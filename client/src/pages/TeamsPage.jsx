    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold tracking-tight flex items-center gap-3">
          <Shield className="w-6 h-6 text-gold" /> 球队中心
        </h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="搜索球队..." value={search}
            ...
            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-white/25
              focus:outline-none focus:border-gold/40 transition-colors" />
        </div>
        <div className="flex gap-1 overflow-x-auto">
          <button onClick={() => setSelectedGroup('')}
            className={`px-3.5 py-2.5 rounded-xl text-sm whitespace-nowrap font-medium transition-all ${
              !selectedGroup ? 'bg-gold text-dark shadow-lg shadow-gold/20' : 'glass-card !py-2.5 text-white/40 hover:text-white/80'
            }`}>全部</button>
          {groups.map(g => (
            <button key={g} onClick={() => setSelectedGroup(g)}
              className={`px-3.5 py-2.5 rounded-xl text-sm whitespace-nowrap font-medium transition-all ${
                selectedGroup === g ? 'bg-gold text-dark shadow-lg shadow-gold/20' : 'glass-card !py-2.5 text-white/40 hover:text-white/80'
              }`}>{g}组</button>
          ))}
        </div>
      </div>
      ...
      {teamsLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {filtered.map(team => (
            <Link key={team.id} to={`/team/${team.id}`} className="glass-card text-center group match-card-hover">
              <FlagImage teamName={team.name} size="lg" />
              <h3 className="font-bold text-sm mt-3 group-hover:text-gold transition-colors">{team.name_cn}</h3>
              <p className="text-[11px] text-white/30 mt-0.5">{team.name}</p>
